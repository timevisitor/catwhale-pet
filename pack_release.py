...
【第一步：让它自己检查环境（v0.1.3 起）】
  第一次启动时，桌宠会自动做一次**快检**（不联网、不扫盘），看两样东西齐不齐：
      · 聊天要用的 DeepSeek Harness    · 文件搜索要用的 Everything
  缺哪样就弹一个「环境自检」窗口，每一项都带按钮：
    · Everything 没在运行 →「自动下载并部署」：从官方源下载便携版、**对照官方 SHA256 校验**、
                           解压到你选的目录并顺手启动；也可以「用检测到的那份」（它会自动找你
                           下载过/装过的 Everything），或「打开官网下载页」自己装。
    · Harness 没就绪   →「一键部署」：浅克隆官方仓库到你选的目录 + 自动装依赖（默认走 npmmirror
                           国内镜像，带实时日志/进度、可随时取消）；已经有仓库就点「浏览…」指定，
                           或只点「在这份仓库上装依赖」。
  不想现在弄就点「关闭」，桌宠其它功能照常可用；勾上「以后别再自动提示」就不会再弹。
  随时可重开：**托盘图标右键 → 🩺 环境自检…**（设置面板里也有入口）。
  说明：Everything 与 Harness 都**不随包分发**，都是你在自己机器上按需获取；
        桌宠只从各自的官方地址取，下载完成会校验官方摘要后才使用。
【一、手动准备 DeepSeek 聊天（向导没装成时再看这里）】
...
【常见问题】
  ...
  文件搜索没结果 / 提示“未检测到 Everything”
      → Everything 没在运行：启动 Everything（托盘里能看到它的图标）后重试；也可以用托盘菜单 → 🩺 环境自检… 里的一键部署/启动按钮。
""")
